headers = {
    "Content-Type": "application/json"
}
user_body = {
    "firstName": "Omar",
    "phone": "+11234567890",
    "address": "123 Elm Street, Hilltop"
}

Autorization = {
    "authToken":'5d47749d-13da-4256-9eb0-57ab12fbca80'
}
def get_kit_body(name):
    reurn kit_bodies[name].copy()

ki_bodies = {
    "one_characer": {"name": "a"},
    "maximus_characters": {"name": ("")},
    "empty_sring": {"name": ""},
    "maximum_plus_one_characters": {"name": ("")},
    "special_characters": {"name": ""%@","},
    "spaces": {"name": " A Aaa "},
    "numbers": {"name": "123"},
    "empty_name": {"name": ""},
    "differet_characers": {"name": 123}
}